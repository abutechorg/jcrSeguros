...
class DeviceController extends ReaxiumAPIController
{
    ...
    /**
     * Delete a device from de systems
     * @param $deviceId
     */
                ...
                if (isset($business_id)) {
                    ...
                    if (trim($filter) != '') {
                        ...
                        $deviceFound = $devicesTable->find()
                            ...
                            ->contain(array("Status", "Business"))->order(array($sortedBy . ' ' . $sortDir));
                    } else {
                        $deviceFound = $devicesTable->find()
    ...
    private function getDeviceFilterBusinessWithPaginate($business_id, $filter, $sortedBy, $sortDir)
    {
        ...
        $query->join(array(
            ...
            'status'=> array(
                'table'=>'status',
                'type'=>'INNER',
                'conditions'=>'ReaxiumDevice.status_id = status.status_id'
            )
        ));
    ...
    public function createDevice()
    {
        ...
        if (parent::validReaxiumJsonHeader($jsonObject)) {
            ...
            try {
                ...
                $business_master_id = !isset($jsonObject['ReaxiumParameters']['ReaxiumDevice']['business_master_id']) ?
                    null : $jsonObject['ReaxiumParameters']['ReaxiumDevice']['business_master_id'];
                    ...
                    if ($device_id == null) {
                        ...
                        if ($validateSerial) {
                            ...
                            if (isset($result)) {
                                ...
                                //se agrega la relacion del dispositivo con el business master del usuario
                                $this->addADeviceRelationWithABusiness($result['device_id'],$business_master_id);
                        ...
                        //se agrega la relacion del dispositivo con el business master del usuario
                        $this->addADeviceRelationWithABusiness($device_id,$business_master_id);
     ...
     * @param $businessId
     */
    private function addADeviceRelationWithABusiness($deviceId, $businessId){
        $deviceBusinessTable = TableRegistry::get("DeviceBusiness");
        $businessIdFound = $deviceBusinessTable->findByBusinessIdAndDeviceId($businessId,$deviceId);
        if($businessIdFound->count() == 0){
            $deviceRelation = $deviceBusinessTable->newEntity();
            $deviceRelation->device_id = $deviceId;
            $deviceRelation->business_id = $businessId;
            $deviceBusinessTable->save($deviceRelation);
        }
    }
