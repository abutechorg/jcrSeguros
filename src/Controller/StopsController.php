...
class StopsController extends ReaxiumAPIController
{
    ...
    public function createStops()
    {
        ...
        Log::info("Create Stops Service invoked");
        parent::setResultAsAJson();
        $response = parent::getDefaultReaxiumMessage();
        $jsonObject = parent::getJsonReceived();
        $validate = true;
    ...
    private function createStopsTransactional($stopTable,$arrayObj){
        ...
        try{
            ...
            $businessStopsTable = TableRegistry::get("BusinessStops");

            $conn->transactional(function() use($stopTable,$arrayObj,$businessStopsTable){
                ...
                foreach ($arrayObj as $entity) {
                    ...
                    $stopsObj = $stopTable->save($stopData);


                    if($entity['business']['business_id'] != 0){
                        $businessStopData = $businessStopsTable->newEntity();
                        $businessStopData->business_id = $entity['business']['business_id'];
                        $businessStopData->id_stop = $stopsObj['id_stop'];
                        $businessStopsTable->save($businessStopData);
                    }
